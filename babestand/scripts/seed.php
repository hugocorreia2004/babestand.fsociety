...
// 5. Verificar/Criar administradores
echo "\nVerificando administradores...\n";

$admins = [
    ['name' => 'Hugo Correia', 'email' => 'hugo.correia@fsociety.pt', 'password' => 'Hugo2025!'],
    ['name' => 'Ryan Barbosa', 'email' => 'ryan.barbosa@fsociety.pt', 'password' => 'Ryan2025!']
];

foreach ($admins as $admin) {
    $existingAdmin = $db->fetch("SELECT id FROM users WHERE email = ?", [$admin['email']]);

    if (!$existingAdmin) {
        $password = password_hash($admin['password'], PASSWORD_ARGON2ID);

        $db->query(
            "INSERT INTO users (name, email, password, role_id, is_active, created_at)
             VALUES (?, ?, ?, 1, 1, NOW())",
            [$admin['name'], $admin['email'], $password]
        );
        echo "  + Admin criado: {$admin['email']}\n";
    } else {
        echo "  Admin já existe: {$admin['email']}\n";
    }
}
